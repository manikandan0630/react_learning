      ...
      <div className="flex flex-col justify-center items-center bg-light  h-svh">
        ...
        <div className="w-9/12 rounded border shadow-lg  bg-white p-4 ">
          <div className="flex justify-end">
            <Link to="/create" className="rounded px-5 py-2 bg-green-500 text-white font-bold">Add +</Link>
          </div>
          <table className="  ">
            <thead >
              <tr className="px-0">
            ...
            <tbody>
              {
                data.map((ele,ind)=>(
                  <tr key={ind} className="mt-5">
                    <td className="px-5 py-2 mt-3">{ele.id}</td>
                    ...
                    <td className=" flex gap-x-2 mt-3 ms-9 text-white font-bold ">
                      ...
                      <Link to={`/update/${ele.id}`} className="rounded py-3 px-3 bg-blue-700  "><LuPencil /></Link>
                      <Link  onClick={()=>handleDelete(ele.id)} className="rounded py-3 px-3 bg-red-700 ]" ><FaRegTrashAlt /></Link>
                    </td>
                  </tr>
                ))
              }
